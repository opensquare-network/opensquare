...
RPC_MANUAL_JSON = './scripts/type-js/rpc_manual.json'
TYPES_MANUAL_JSON = './scripts/type-js/types_manual.json'

MANUAL = {}
# Read the types crated manually and convert the under_score_case to camelCase.
with open(TYPES_MANUAL_JSON) as json_file:
...
# Find all *.rs in project, following the gitignore rule.
        ...
        with open(fname, 'r') as reader:
            ...
            for line in lines:
                ...
                if '[rpc(name =' in line:
                    ...
                    #  Only handle the specific RPC, starting with x
...
def write_types_and_rpc():
    ...
    with open(RPC_MANUAL_JSON) as json_file:
        RPC_MANUAL = json.load(json_file)

    os.chdir("./scripts/type-js")
    write_json(output, 'res/types.json')
    ...
    write_json(rpc_dict, 'res/rpc.json')
